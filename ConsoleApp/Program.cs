...
namespace ConsoleApp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //{
            //    Console.WriteLine(Db.Parts[i].Type);
            //}
            ...
            //{
            //    Console.WriteLine(Db.Parts[i].Company);
            //}

            //Console.ReadLine();


            // Console.WriteLine("Welcome to our company for PC parts what would you like to buy:");
         
             
            while(true)
            {
               
                ServicesFunction service = new ServicesFunction();
                var res = service.CooseOption();
                List<Item> products = new List<Item>();
               
                if (res == 1) products.AddRange(Db.Parts);
                if (res == 2) products.AddRange(Db.Modules);
                if (res == 3) products.AddRange(Db.Configurations);
               
                while (true)
                {
                   
                    var filter = service.CooseOptionFilterParts();
                    if(filter == 1)
                    {
                        while(true)
                        {
                            service.PrintAllProducts(products);
                            var nav = service.Navigation();
                            if (nav == 1) continue;
                            if (nav == 2) break;
                            if (nav == 3)
                            {
                                Cart.PrintProductsInCard();
                                Console.ReadLine();
                                break;
                            }
                            if (nav == 4) { Console.WriteLine("Check out"); Console.ReadLine(); break; }
                        }
                    ...
                    if (filter == 3)
                    {
                        while (true)
                        {
                            if (res == 1) service.PrintTypeProductsForParts(Db.Parts);
                            if (res == 2) service.PrintTypeProductsForModules(Db.Modules);
                            if (res == 3) service.PrintTypeProductsForConfiguration(Db.Configurations);
